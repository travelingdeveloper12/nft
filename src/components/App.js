...
import AllNFTMarketplace from "./AllNFTMarketplace/AllNFTMarketplace";
...
class App extends Component {
  ...
  loadBlockchainData = async () => {
    ...
    if (accounts.length === 0) {
      ...
      const networkData = NFTJson.networks[networkId];
      if (networkData) {
        this.setState({ loading: true });
        const nftsContract = web3.eth.Contract(
          NFTJson.abi,
          networkData.address
        );
        this.setState({ nftsContract });
        this.setState({ contractDetected: true });
        const nftsCount = await nftsContract.methods
          .nftCounter()
          .call();
        this.setState({ nftsCount });
        for (var i = 1; i <= nftsCount; i++) {
          const nft = await nftsContract.methods
            .allNfts(i)
            .call();
          this.setState({
        ...
        let totalTokensMinted = await nftsContract.methods
          .getNumberOfTokensMinted()
          .call();
        totalTokensMinted = totalTokensMinted.toNumber();
        this.setState({ totalTokensMinted });
        let totalTokensOwnedByAccount = await nftsContract.methods
  ...
  mintMyNFT = async (colors, name, tokenPrice) => {
    ...
    for (let i = 0; i < colorsArray.length; i++) {
      if (colorsArray[i] !== "") {
        let colorIsUsed = await this.state.nftsContract.methods
    ...
    const nameIsUsed = await this.state.nftsContract.methods
    ...
    if (colorsUsed.length === 0 && !nameIsUsed) {
      ...
      let previousTokenId;
      previousTokenId = await this.state.nftsContract.methods
        .nftCounter()
        .call();
      ...
      this.state.nftsContract.methods
        .mintMyNFT(name, tokenURI, price, colorsArray)
          ...
          <>
            <HashRouter basename="/">
              ...
              <Route
                path="/queries"
                render={() => (
                  <Queries nftsContract={this.state.nftsContract} />
                )}
              />
            </HashRouter>
          </>
